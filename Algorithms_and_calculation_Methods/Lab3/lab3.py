    ...
    try:
        ...
        for degree in degrees:
            ...
            y_interpolated_degree = np.array(
                [
                    aitken_interpolation(x_nodes_degree, y_nodes_degree, x)
                    for x in x_values
                ]
            )
        ...
        error_table = ft.DataTable(
            columns=[
                ft.DataColumn(ft.Text("Ступінь", color="black")),
                ft.DataColumn(ft.Text("Інтерполяційне значення", color="black")),
...
def main(page: ft.Page):
    ...
    input_field_n = ft.TextField(
        label="Введіть n (кількість вузлових точок)", color="black"
    )
