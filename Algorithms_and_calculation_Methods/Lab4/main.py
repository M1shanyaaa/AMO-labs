...
def main(page: ft.Page):
    ...
    def find_sign_changes(a, b, step=0.01):
        ...
        while x < b:
            x_next = x + step
            f_x, f_x_next = evaluate_function(equation, x), evaluate_function(
                equation, x_next
            )
    ...
    def chord_method_click(e):
        try:
            ...
            if not sign_change_intervals:
                result_text.value = (
                    "Помилка: Не знайдено змін знаку функції в заданому інтервалі"
                )
                page.update()
                return
            ...
            for interval in sign_change_intervals:
                ...
                if root is not None:
                    ...
                    process_info.controls.append(
                        ft.Text(
                            f"Інтервал: ({interval[0]:.4f}, {interval[1]:.4f}) → Корінь: {root:.6f}"
                        )
                    )

            result_text.value = (
                f"Знайдені корені: {', '.join(f'{r:.8f}' for r in roots)}"
            )
            iterations_text.value = f"Кількість знайдених коренів: {len(roots)}"

            plot_buffer = generate_plot(a, b, roots, all_history)
            function_plot.src_base64 = base64.b64encode(plot_buffer.read()).decode(
                "utf-8"
            )
    ...
    calculate_button = ft.ElevatedButton(
        "Знайти корені", on_click=chord_method_click, icon=ft.Icons.CALCULATE, width=200
    )
    ...
    page.add(
        ft.Column(
            [
                ...
                ft.Divider(),
                ft.Text(
                    "Розв'язання нелінійних рівнянь методом хорд",
                    size=20,
                    weight=ft.FontWeight.BOLD,
                ),
