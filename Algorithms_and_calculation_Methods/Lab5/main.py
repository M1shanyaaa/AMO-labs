...
def main(page: ft.Page):
    ...
    def create_matrix_inputs(e):
        try:
            ...
            example_values = [[20, 1, 2], [3, 20, 1], [2, 3, 20]] if size == 3 else None
            ...
            for i in range(size):
                ...
                for j in range(size):
                    value = (
                        ...
                        if example_values
                        and i < len(example_values)
                        and j < len(example_values[i])
                        else f"{np.random.randint(1, 10)}"
                    )
                    row.controls.append(
                        ft.TextField(
                            width=100,
            ...
            vector_inputs.current.controls.append(
                ft.Text("Вектор вільних членів b:", weight=ft.FontWeight.BOLD)
            )
            ...
            for i in range(size):
                value = (
                    str(example_b[i])
                    if example_b and i < len(example_b)
                    else f"{np.random.randint(0, 10)}"
                )
    ...
    def calculate_solution(e):
        try:
            ...
            for i in range(size):
                for j in range(size):
                    A[i, j] = float(
                        matrix_inputs.current.controls[i + 1].controls[j].value
                    )
            ...
            if violations:
                ...
                show_snackbar(
                    f"Попередження: Немає діагональної домінації в рядках: {rows}"
                )
                return
            ...
            results.current.controls.append(
                ft.Text(f"Кількість ітерацій: {iterations}")
            )
            results.current.controls.append(ft.Text(f"Досягнута похибка: {error:.6f}"))
            results.current.controls.append(
                ft.Text("Розв'язок системи:", weight=ft.FontWeight.BOLD)
            )
            for i in range(size):
                results.current.controls.append(
                    ft.Text(f"x{i + 1} = {solution[i]:.6f}")
                )
    ...
    def sor_method(A, b, omega, initial_guess, max_iterations, tolerance):
        ...
        for iteration in range(max_iterations):
            ...
            for i in range(n):
                sigma = sum(
                    A[i, j] * (x[j] if j < i else x_old[j]) for j in range(n) if j != i
                )
            ...
            controls=[
                ...
                ft.Row(
                    controls=[
                        ...
                        ft.TextField(
                            ref=omega,
                            label="Параметр релаксації ω (1 < ω < 2)",
                            width=200,
                            keyboard_type=ft.KeyboardType.NUMBER,
                            value="1.1",
                        ),
